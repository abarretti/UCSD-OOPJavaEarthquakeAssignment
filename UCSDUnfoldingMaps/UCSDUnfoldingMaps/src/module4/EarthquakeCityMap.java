...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() 
	{
		...
		HashMap<String, Integer> countryEarthquakeCount = new HashMap<>();
		for (Marker quakeMarker : quakeMarkers) {
			String country = (String)quakeMarker.getProperty("country");
			if (country == null) {
				country = "OCEAN QUAKES";
			}
			if (countryEarthquakeCount.containsKey(country)) {
				countryEarthquakeCount.replace(country, countryEarthquakeCount.get(country) + 1);
			} else {
				countryEarthquakeCount.put(country, 1);
			}
		}
		
		for (String key : countryEarthquakeCount.keySet()) {
			System.out.println(key + ": " + countryEarthquakeCount.get(key));
		}
	}
